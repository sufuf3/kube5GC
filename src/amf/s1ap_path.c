...
status_t s1ap_open(void)
{
    ...
    rv = s1ap_server_list(&amf4g_self()->s1ap_list, type);
    d_assert(rv == CORE_OK, return CORE_ERROR,);
    rv = s1ap_server_list(&amf4g_self()->s1ap_list6, type);
    d_assert(rv == CORE_OK, return CORE_ERROR,);
...
status_t s1ap_close()
{
    s1ap_delete_list(&amf4g_self()->s1ap_list);
    s1ap_delete_list(&amf4g_self()->s1ap_list6);
        ...
        enb_ue_t *source_ue)
{
    ...
    rv = s1ap_build_mme_cp_relocation_indication(&s1apbuf, source_ue);
    d_assert(rv == CORE_OK && s1apbuf, return CORE_ERROR, "s1ap build error");
...
status_t s1ap_send_to_enb(amf4g_enb_t *enb, pkbuf_t *pkbuf, c_uint16_t stream_no)
{
...
status_t s1ap_send_to_enb_ue(enb_ue_t *enb_ue, pkbuf_t *pkbuf)
{
    ...
    return s1ap_send_to_enb(enb, pkbuf, amf4g_ue->ostream_id);
}
...
status_t s1ap_send_to_esm(amf4g_ue_t *amf4g_ue, pkbuf_t *esmbuf)
{
    event_t e;
        ...
        S1AP_ProcedureCode_t procedureCode, S1AP_NAS_PDU_t *nasPdu)
{
    ...
    if (enb_ue->amf4g_ue)
    {
        d_assert(nas_security_decode(
            enb_ue->amf4g_ue, security_header_type, nasbuf) == CORE_OK,
    ...
    else if (h->protocol_discriminator == NAS_PROTOCOL_DISCRIMINATOR_ESM)
    {
        ...
        d_assert(amf4g_ue, return CORE_ERROR, "Null param");
        s1ap_send_to_esm(amf4g_ue, nasbuf);
    }
    else
...
status_t s1ap_send_initial_context_setup_request(amf4g_ue_t *amf4g_ue)
{
    status_t rv;
    pkbuf_t *s1apbuf = NULL;
    ...
    rv = s1ap_build_initial_context_setup_request(&s1apbuf, amf4g_ue, NULL);
    d_assert(rv == CORE_OK && s1apbuf, return CORE_ERROR, "s1ap build error");
...
status_t s1ap_send_ERAB_release_command(amf4g_enb_t *enb,
...
status_t s1ap_send_path_switch_ack(amf4g_ue_t *amf4g_ue)
{
    ...
    rv = s1ap_build_path_switch_ack(&s1apbuf, amf4g_ue);
    d_assert(rv == CORE_OK && s1apbuf, return CORE_ERROR, "s1ap build error");
            ...
            *source_totarget_transparentContainer)
{
    ...
    rv = s1ap_build_handover_request(&s1apbuf, amf4g_ue, target_ue,
	...
	amf4g_ue_t *amf4g_ue)
{
    ...
    rv = s1ap_build_connection_establishment_indication(&s1apbuf, mme_ue_s1ap_id, enb_ue_s1ap_id, amf4g_ue);
    d_assert(rv == CORE_OK && s1apbuf, return CORE_ERROR, "s1ap build error");
	...
	S1AP_S_TMSI_t *S_TMSI)
{
    ...
    rv = s1ap_build_ue_information_transfer(&s1apbuf, amf4g_ue, S_TMSI);
    d_assert(rv == CORE_OK && s1apbuf, return CORE_ERROR, "s1ap build error");
...
status_t s1ap_send_ue_context_modification_request(
	enb_ue_t *enb_ue)
{
    status_t rv;
    pkbuf_t *s1apbuf = NULL;
    ...
    /* build UE Context Modification Request packet */
    rv = s1ap_build_ue_context_modification_request(&s1apbuf, enb_ue);
    d_assert(rv == CORE_OK && s1apbuf, return CORE_ERROR, "s1ap build error");
