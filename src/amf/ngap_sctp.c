#define TRACE_MODULE _ngap_sctp
...
static int ngap_accept_handler(sock_id sock, void *data);

status_t ngap_init(int sctp_streams, c_uint16_t port)
{
...
status_t ngap_server(sock_node_t *snode, int type)
{
    status_t rv;
    ...
    rv = sock_register(snode->sock, ngap_accept_handler, NULL);
    d_assert(rv == CORE_OK, return CORE_ERROR,);

    d_trace(1, "ngap_server() [%s]:%d\n",
            CORE_ADDR(snode->list, buf), CORE_PORT(snode->list));

    return CORE_OK;
}

status_t ngap_delete(sock_id sock)
{
    d_assert(sock, return CORE_ERROR,);
    return sock_delete(sock);
}

status_t ngap_send(sock_id sock, pkbuf_t *pkbuf,
        c_sockaddr_t *addr, c_uint16_t stream_no)
{
    ...
    sent = core_sctp_sendmsg(sock, pkbuf->payload, pkbuf->len,
            addr, SCTP_NGAP_PPID, stream_no);
    d_trace(50, "[NGAP] SEND[%d] : ", sent);
...
status_t ngap_recv(sock_id id, pkbuf_t *pkbuf)
{
    int size;
...
static int ngap_accept_handler(sock_id id, void *data)
{
    char buf[CORE_ADDRSTRLEN];
    status_t rv;
    sock_id new;
    ...
    if (rv == CORE_OK)
    {
        ...
        d_trace(1, "gNB-NG accepted[%s]:%d in ng_path module\n", 
            CORE_ADDR(addr, buf), CORE_PORT(addr));

        event_set(&e, AMF_EVT_NGAP_LO_ACCEPT);
...
int ngap_recv_handler(sock_id sock, void *data)
{
    pkbuf_t *pkbuf;
    int size;
    event_t e;
        ...
        switch(not->sn_header.sn_type) 
        {
            case SCTP_ASSOC_CHANGE :
            {
                ...
                if (not->sn_assoc_change.sac_state == SCTP_COMM_UP)
                {
                    ...
                    event_set(&e, AMF_EVT_NGAP_LO_SCTP_COMM_UP);
                    ...
                    if (amf4g_event_send(&e) != CORE_OK)
                    {
                        d_error("Event AMF_EVT_NGAP_LO_SCTP_COMM_UP failed");
                        CORE_FREE(addr);
                    }
                }
                        ...
                        not->sn_assoc_change.sac_state == SCTP_COMM_LOST)
                {
                    ...
                    event_set(&e, AMF_EVT_NGAP_LO_CONNREFUSED);
                    event_set_param1(&e, (c_uintptr_t)sock);
                    event_set_param2(&e, (c_uintptr_t)addr);

                    if (amf4g_event_send(&e) != CORE_OK)
                    {
                        d_error("Event AMF_EVT_NGAP_LO_CONNREFUSED failed");
                        CORE_FREE(addr);
                    }
    ...
    else if (flags & MSG_EOR)
    {
        ...
        event_set(&e, AMF_EVT_NGAP_MESSAGE);
        event_set_param1(&e, (c_uintptr_t)sock);
        event_set_param2(&e, (c_uintptr_t)addr);
        event_set_param3(&e, (c_uintptr_t)pkbuf);
        if (amf4g_event_send(&e) != CORE_OK)
        {
            d_error("Event AMF_EVT_NGAP_MESSAGE failed");
            pkbuf_free(pkbuf);
            CORE_FREE(addr);
        }
