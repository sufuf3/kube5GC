...
status_t ngap_send_to_gnb(amf_gnb_t *gnb, pkbuf_t *pkbuf, c_uint16_t stream_no)
{
    char buf[CORE_ADDRSTRLEN];
    status_t rv;

    d_assert(gnb, return CORE_ERROR,);
    d_assert(pkbuf, return CORE_ERROR,);
    d_assert(gnb->sock, return CORE_ERROR,);

    d_trace(5, "    IP[%s] GNB_ID[%d]\n",
            CORE_ADDR(gnb->addr, buf), gnb->gnb_id);

    rv = ngap_send(gnb->sock, pkbuf,
            gnb->sock_type == SOCK_STREAM ? NULL : ngenb->addr,
            stream_no);
    if (rv != CORE_OK)
    {
