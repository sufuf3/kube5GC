...
status_t nas_send_to_enb(amf4g_ue_t *amf4g_ue, pkbuf_t *pkbuf)
{
        ...
        amf4g_ue_t *amf4g_ue, nas_esm_message_container_t *esm_message_container)
{
    ...
    d_assert(s1ap_send_to_esm(amf4g_ue, esmbuf) == CORE_OK, return CORE_ERROR,
            "s1ap_send_to_esm failed");

    return CORE_OK;
}

status_t nas_send_to_downlink_nas_transport(amf4g_ue_t *amf4g_ue, pkbuf_t *pkbuf)
{
    status_t rv;
    ...
    rv = nas_send_to_enb(amf4g_ue, s1apbuf);
    d_assert(rv == CORE_OK, return CORE_ERROR, "nas send error");

    return CORE_OK;
}

status_t nas_send_attach_accept(amf4g_ue_t *amf4g_ue)
{
    status_t rv;
    ...
    d_assert(amf4g_sess_next(sess) == NULL,
    ...
    rv = emm_build_attach_accept(&emmbuf, amf4g_ue, esmbuf);
    d_assert(rv == CORE_OK && emmbuf, 
            pkbuf_free(esmbuf); return CORE_ERROR, "emm build error");

    rv = s1ap_build_initial_context_setup_request(&s1apbuf, amf4g_ue, emmbuf);
    d_assert(rv == CORE_OK && s1apbuf, 
...
status_t nas_send_attach_reject(amf4g_ue_t *amf4g_ue,
...
status_t nas_send_identity_request(amf4g_ue_t *amf4g_ue)
{
    status_t rv;
    pkbuf_t *emmbuf = NULL;
    ...
    rv = emm_build_identity_request(&emmbuf, amf4g_ue);
...
status_t nas_send_authentication_reject(amf4g_ue_t *amf4g_ue)
{
    status_t rv;
    pkbuf_t *emmbuf = NULL;
...
status_t nas_send_detach_accept(amf4g_ue_t *amf4g_ue)
{
    status_t rv;
    enb_ue_t *enb_ue = NULL;
    pkbuf_t *emmbuf = NULL;
    ...
    if (amf4g_ue->nas_eps.detach.switch_off == 0)
    {
        rv = emm_build_detach_accept(&emmbuf, amf4g_ue);
        d_assert(rv == CORE_OK && emmbuf, return CORE_ERROR,
                "nas_build_detach_accept failed"); 
    ...
    amf4g_sess_t *sess, nas_esm_cause_t esm_cause)
{
    ...
    rv = nas_send_to_downlink_nas_transport(amf4g_ue, esmbuf);
...
status_t nas_send_esm_information_request(amf4g_bearer_t *bearer)
{
    status_t rv;
...
status_t nas_send_activate_default_bearer_context_request(amf4g_bearer_t *bearer)
{
    status_t rv;
...
status_t nas_send_activate_all_dedicated_bearers(amf4g_bearer_t *default_bearer)
{
    status_t rv;

    d_assert(default_bearer, return CORE_ERROR, "Null param");

    amf4g_bearer_t *dedicated_bearer = amf4g_bearer_next(default_bearer);
    while(dedicated_bearer)
    {
        ...
        dedicated_bearer = amf4g_bearer_next(dedicated_bearer);
    }
...
status_t nas_send_deactivate_bearer_context_request(amf4g_bearer_t *bearer)
{
    status_t rv;
    pkbuf_t *esmbuf = NULL, *s1apbuf = NULL;
    amf4g_ue_t *amf4g_ue = NULL;
        ...
        amf4g_ue_t *amf4g_ue, S1AP_ProcedureCode_t procedureCode)
{
    ...
    rv = emm_build_tau_accept(&emmbuf, amf4g_ue);
...
status_t nas_send_tau_reject(amf4g_ue_t *amf4g_ue, nas_emm_cause_t emm_cause)
{
    status_t rv;
    pkbuf_t *emmbuf = NULL;
    ...
    /* Build TAU reject */
    rv = emm_build_tau_reject(&emmbuf, emm_cause, amf4g_ue);
    d_assert(rv == CORE_OK, return CORE_ERROR, "emm build error");
...
status_t nas_send_service_reject(amf4g_ue_t *amf4g_ue, nas_emm_cause_t emm_cause)
{
    status_t rv;
    pkbuf_t *emmbuf = NULL;
    ...
    rv = emm_build_service_reject(&emmbuf, emm_cause, amf4g_ue);
    d_assert(rv == CORE_OK, return CORE_ERROR, "emm build error");
