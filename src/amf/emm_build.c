        ...
        pkbuf_t **emmbuf, amf4g_ue_t *amf4g_ue, pkbuf_t *esmbuf)
{
    ...
    /* Set T3412 */
    eps_attach_result->result = amf4g_ue->nas_eps.attach.attach_type;
    ...
    served_tai_index = amf4g_find_served_tai(&amf4g_ue->tai);
    d_trace(5, "    SERVED_TAI_INDEX[%d]\n", served_tai_index);
    d_assert(served_tai_index >= 0 &&
            served_tai_index < MAX_NUM_OF_SERVED_TAI, return CORE_ERROR,
            "Cannot find Served TAI. Check 'amf4g.tai' configuration");
    nas_tai_list_build(&attach_accept->tai_list,
            &amf4g_self()->served_tai[served_tai_index].list0,
            &amf4g_self()->served_tai[served_tai_index].list2);
    ...
    d_trace(5, "    %s GUTI[G:%d,C:%d,M_TMSI:0x%x] IMSI:[%s]\n",
            amf4g_ue->guti_present ? "[V]" : "[N]",
            amf4g_ue->guti.mme_gid, amf4g_ue->guti.mme_code,
            amf4g_ue->guti.m_tmsi, amf4g_ue->imsi_bcd);
    if (amf4g_ue->guti_present)
    {
        ...
        memcpy(&guti->guti.plmn_id, &amf4g_ue->guti.plmn_id, PLMN_ID_LEN);
        guti->guti.mme_gid = amf4g_ue->guti.mme_gid;
        guti->guti.mme_code = amf4g_ue->guti.mme_code;
        guti->guti.m_tmsi = amf4g_ue->guti.m_tmsi;
    }
    amf4g_ue->guti_present = 0;
        ...
        pkbuf_t **emmbuf, amf4g_ue_t *amf4g_ue)
{
    ...
    for (i = 0; i < amf4g_self()->num_of_integrity_order; i++)
    {
        if (amf4g_ue->ue_network_capability.eia & 
                (0x80 >> amf4g_self()->integrity_order[i]))
        {
            amf4g_ue->selected_int_algorithm = amf4g_self()->integrity_order[i];
            break;
        }
    }
    for (i = 0; i < amf4g_self()->num_of_ciphering_order; i++)
    {
        if (amf4g_ue->ue_network_capability.eea & 
                (0x80 >> amf4g_self()->ciphering_order[i]))
        {
            amf4g_ue->selected_enc_algorithm = amf4g_self()->ciphering_order[i];
            break;
        }
    }

    selected_nas_security_algorithms->type_of_integrity_protection_algorithm =
        amf4g_ue->selected_int_algorithm;
    selected_nas_security_algorithms->type_of_ciphering_algorithm =
        amf4g_ue->selected_enc_algorithm;

    nas_key_set_identifier->tsc = 0;
    nas_key_set_identifier->nas_key_set_identifier = 0;

    replayed_ue_security_capabilities->eea = amf4g_ue->ue_network_capability.eea;
    replayed_ue_security_capabilities->eia = amf4g_ue->ue_network_capability.eia;
    replayed_ue_security_capabilities->uea = amf4g_ue->ue_network_capability.uea;
    replayed_ue_security_capabilities->uia = 
        amf4g_ue->ue_network_capability.uia & 0x7f;
    replayed_ue_security_capabilities->gea = 
        (amf4g_ue->ms_network_capability.gea1 << 6) | 
        amf4g_ue->ms_network_capability.extended_gea;
...
status_t emm_build_detach_accept(pkbuf_t **emmbuf, amf4g_ue_t *amf4g_ue)
{
    status_t rv;
    nas_message_t message;
...
status_t emm_build_tau_accept(pkbuf_t **emmbuf, amf4g_ue_t *amf4g_ue)
{
    nas_message_t message;
    ...
    tau_accept->eps_update_result.result = amf4g_ue->nas_eps.update.update_type;
    ...
    d_assert(nas_security_encode(emmbuf, amf4g_ue, &message) == CORE_OK && 
            *emmbuf,,);
