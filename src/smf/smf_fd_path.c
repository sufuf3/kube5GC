#define TRACE_MODULE _smf_fd_path
...
pool_declare(smf_gx_sess_pool, struct sess_state, MAX_POOL_OF_DIAMETER_SESS);
...
static __inline__ struct sess_state *new_state(os0_t sid)
{
    ...
    pool_alloc_node(&smf_gx_sess_pool, &new);
...
status_t smf_fd_init(void)
{
    int ret;
	struct disp_when data;

    pool_init(&smf_gx_sess_pool, MAX_POOL_OF_DIAMETER_SESS);

    ret = fd_init(FD_MODE_CLIENT|FD_MODE_SERVER,
                smf_self()->fd_conf_path, smf_self()->fd_config);
...
void smf_fd_final(void)
{
    ...
    if (pool_used(&smf_gx_sess_pool))
        d_error("%d not freed in smf_gx_sess_pool[%d] of GX-SM",
                pool_used(&smf_gx_sess_pool), pool_size(&smf_gx_sess_pool));
    d_trace(9, "%d not freed in smf_gx_sess_pool[%d] of GX-SM\n",
            pool_used(&smf_gx_sess_pool), pool_size(&smf_gx_sess_pool));

    pool_final(&smf_gx_sess_pool);
}
