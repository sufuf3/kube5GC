...
status_t smf_compile_packet_filter(smf_rule_t *smf_rule, c_int8_t *description)
{
    ...
    d_assert(smf_rule, return CORE_ERROR, "Null param");
			...
			l > 0 ; l -= F_LEN(cmd) , cmd += F_LEN(cmd))
    {
		switch(cmd->opcode) 
        {
            ...
            case O_IP6_SRC_MASK:
            {
                ...
                memcpy(smf_rule->ip.local.addr, a, IPV6_LEN);
                if (cmd->opcode == O_IP6_SRC_MASK)
                    memcpy(smf_rule->ip.local.mask, a+4, IPV6_LEN);
                else
            ...
            case O_IP6_DST_MASK:
            {
                ...
                memcpy(smf_rule->ip.remote.addr, a, IPV6_LEN);
                if (cmd->opcode == O_IP6_DST_MASK)
                    memcpy(smf_rule->ip.remote.mask, a+4, IPV6_LEN);
                else
...
smf_bearer_t *smf_bearer_find_by_packet(pkbuf_t *pkt)
{
    ...
    {
        smf_sess_t *sess = smf_sess_this(hi);
