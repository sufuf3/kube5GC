...
if [ "$SYSTEM" = "Linux" ]; then
    ...
    if [ "$SYSTEM" = "Darwin" ]; then
        if ! test -f /etc/pf.anchors/org.nctu5gc; then
            sudo sh -c "echo 'nat on {en0} from 45.45.0.0/16 to any -> {en0}' > /etc/pf.anchors/org.nctu5gc"
        fi
        pfctl -e -f /etc/pf.anchors/org.nctu5gc
    fi
fi
